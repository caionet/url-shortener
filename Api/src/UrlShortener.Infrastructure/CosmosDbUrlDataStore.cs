...
namespace UrlShortener.Infrastructure
{
    ...
    {
        public async Task AddAsync(ShortenedUrl shortened, CancellationToken ct)
        {
            var document = (ShortenedUrlCosmos)shortened;
           await container.CreateItemAsync(document, new PartitionKey(document.ShortUrl), cancellationToken: ct);
        }
    }
